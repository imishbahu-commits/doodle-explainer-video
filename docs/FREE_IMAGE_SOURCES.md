...
| 1 | **Puter.js** (`puter.ai.txt2img`) | FREE — **anonymous tier works** (20 req/10s, 2 concurrent); free account raises to 30 req/10s, 3 concurrent + monthly allowance that resets monthly | no key — anonymous first; optional **free Puter login** (human popup, ~10 s; **no headless signup exists**) | **Top-tier: GPT Image 2 (quality high), Nano Banana Pro (1K/2K/4K), FLUX.2 Pro, Grok Imagine, SD 3.5** | any browser (phone studio — now the DEFAULT engine) | verified from official docs this session: `puter.ai.txt2img(prompt, {provider, model, quality, ratio})`, `puter.auth.signIn()/isSignedIn()`, rate-limits page (anonymous + free tiers), llms.txt full API |

#!/usr/bin/env python3
"""Render Bataspis Part 2 with exact global-frame synchronization."""
...
W,H,FPS=1376,768,30;START,END=1251,2609;BAR=round(H*.10)
...
CAP={10:'BIGGER THAN ONE WEIRD HEAD',11:"EVOLUTION'S WORKBENCH",12:'EARLY DEVONIAN',13:'NO MODERN OCEAN',14:'JAWS: OPTIONAL|PAIRED FINS: OPTIONAL',15:'NO STANDARD FISH SHAPE',16:'THE GALEASPIDS',17:'CHINA + NORTHERN VIETNAM',18:'RIGID HEADSHIELD',19:'SOFT TRUNK + TAIL|NO MOVABLE JAW'}
...
def badge(im,text,xy,t,start,size=42,fill=INK,bg=PAPER):
 q=pop(t,start)
 if q<=0:return
...
def annotate(im,n,t):
 texts=CAP[n].split('|')
 if len(texts)==1:badge(im,texts[0],(W//2,665),t,.5,42,TEAL if n in {10,12,16,17,18} else RED)
 else:badge(im,texts[0],(340,665),t,.35,37,RED);badge(im,texts[1],(1035,665),t,1.2,37,RED)
def beats():return [x for x in json.loads(BEATS.read_text())['beats'] if x['part']==2]
def prep(p,n):
 im=fit(p);colors=10 if n in {12,14,16,17,18} else 12;return im.quantize(colors=colors,method=Image.Quantize.MEDIANCUT,dither=Image.Dither.NONE).convert('RGB')
def load(bs):
 out={}
 for b in bs:
...
def render(out,poster):
 bs=beats();ps=load(bs);a0=START/FPS;dur=(END-START)/FPS
 cmd=[FFMPEG,'-y','-v','error','-f','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r',str(FPS),'-i','-','-ss',f'{a0:.9f}','-t',f'{dur:.9f}','-i',str(AUDIO),'-filter_complex','[1:a]asetpts=PTS-STARTPTS,volume=-2.36dB[a]','-map','0:v','-map','[a]','-frames:v',str(END-START),'-c:v','libx264','-preset','fast','-crf','19','-pix_fmt','yuv420p','-c:a','aac','-b:a','192k','-ar','44100','-ac','2','-movflags','+faststart','-shortest',str(out)]
 p=subprocess.Popen(cmd,stdin=subprocess.PIPE,stderr=subprocess.PIPE);i=0;wrote=False
 for g in range(START,END):
  while g>=bs[i]['end_frame_exclusive']:i+=1
  im=frame(bs[i],g,ps[bs[i]['number']])
  if poster and not wrote and g>=2190:poster.parent.mkdir(parents=True,exist_ok=True);im.save(poster);wrote=True
